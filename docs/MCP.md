...
- **Startup**: Runs when `ScannerViewModel` starts scanning (Wi‑Fi on), stops when scanning stops. In Pro builds the server stays on while the recording session is active.
