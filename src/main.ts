...
function renderToC() {
  const items = [
    ...
    { id: 'mcp', label: 'MCP' },
    { id: 'privacy', label: 'Privacy', icon: true },
  ...
  <aside class="toc-sidebar hidden xl:block fixed right-6 top-1/2 -translate-y-1/2 z-40 w-36">
    <nav class="flex flex-col gap-0.5 border-l border-gray-800/50 pl-4 py-3 bg-gray-950/70 backdrop-blur-sm rounded-r-lg">
      ${items.map(it => /* html */ `
      <a href="#${it.id}" class="toc-link text-[11px] text-gray-500 hover:text-gray-200 transition-colors duration-200 py-0.5 block leading-snug${it.icon ? ' toc-privacy' : ''}">${it.icon ? `<svg class="w-3 h-3 inline-block mr-1 -mt-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>` : ''}${it.label}</a>
      `).join('')}
    </nav>
  </aside>`
}
